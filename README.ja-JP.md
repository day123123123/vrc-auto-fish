...
私自身、YouTube というプラットフォームの扱いにあまり慣れていないため、動画を投稿することができません。基本的には、各自で手探りしながら使っていただく形になります。もし使い方を十分に理解している方がいて、チュートリアル動画の投稿を手伝っていただけるなら、とてもありがたいです。
...
### 方法0: `VRC Auto Fish Launcher` を使う（大半のユーザー向け）

`VRC Auto Fish Launcher` はダウンローダー兼ランチャーであり、釣り本体そのものではありません。
本体の取得、更新確認、依存関係の準備、起動までをまとめて扱うためのツールで、手動セットアップや更新作業を減らしたい人向けです。

`Launcher` を使う場合は、通常 `启动.bat`、`install.bat`、手動コマンド入力を自分で行う必要はありません。
