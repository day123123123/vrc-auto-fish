...
### 方式零：使用 `VRC Auto Fish Launcher`（适合大多数用户）

`VRC Auto Fish Launcher` 是下载器 / 启动器，不是钓鱼本体本身。
它的作用是帮你自动完成本体下载、更新检查、依赖准备和启动，适合不想手动折腾环境、也不想每次自己拉更新的用户。

如果你下载的是 `Launcher`，一般不需要手动执行 `启动.bat`、`install.bat` 或自己敲命令。
